...
class SmartFeedAlgorithm:
    ...
    def get_score_weights():
        return {
            'compatibility': 0.30, 'availability': 0.20, 'activity': 0.15,
            'safety': 0.10, 'popularity': 0.10, 'distance': 0.15
        }
            ...
            if hasattr(user, 'profile') and user.profile.latitude and user.profile.longitude:
                if candidate_profile.latitude and candidate_profile.longitude:
                    ...
                    if distance is not None:
                        scores['distance'] = 100 if distance < 5 else 80 if distance < 20 else 60 if distance < 50 else 40 if distance < 100 else 20
                    else:
                        scores['distance'] = 50
                else:
                    scores['distance'] = 50
...
def profile_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            profile.refresh_from_db()  # FIXED: Refresh to get updated values
...
# FIXED: Profile completion now includes interests and photos
...
def chat_view(request, match_id):
    ...
    return render(request, 'chat.html', {
        'match': match, 'other_user': other_user, 'messages': msgs,
        'is_paywalled': is_paywalled, 'safe_meets': safe_meets,
...
def knocks_view(request):
    ...
    for knock in knocks:
        try:
            ...
            mutual_interests = list(set(request.user.profile.interests) & set(profile.interests)) if request.user.profile.interests and profile.interests else []
            compatibility = SmartFeedAlgorithm.calculate_profile_score(request.user, profile) if request.user.profile.interests else 0
            enhanced_knocks.append({'knock': knock, 'profile': profile, 'mutual_interests': mutual_interests, 'compatibility': compatibility})
        except Exception:
            continue
...
def safety_center_view(request):
    ...
    safety_tips = ["Always meet in public places", "Share your location with a friend", "Trust your instincts", "Keep personal info private", "Use the check-in feature", "Video chat first", "Don't share financial info"]
    return render(request, 'safety_center.html', {'active_meets': active_meets, 'pending_meets': pending_meets, 'blocked_users': blocked_users, 'safety_tips': safety_tips, 'profile_safety_score': getattr(user.profile, 'safety_score', 100)})
...
# FIXED: SafeMeet now supports confirmation by other person
@login_required
def schedule_safe_meet_view(request, match_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            safemeet.save()
            Message.objects.create(match=match, from_user=request.user, body=f"🛡️ Safety meeting proposed for {safemeet.meeting_time.strftime('%B %d at %I:%M %p')} at {safemeet.location}. Waiting for confirmation.", is_system_message=True)
            messages.success(request, "Safety meeting proposed! Waiting for confirmation.")
            return redirect('chat', match_id=match.id)
    else:
        form = SafeMeetForm()
...
# NEW: Confirm safe meet (for the invited person)
@login_required
def confirm_safe_meet_view(request, safemeet_id):
    ...
    if request.user != safemeet.to_user:
        messages.error(request, "Only the invited person can confirm.")
    ...
    safemeet.save()
    if safemeet.match:
        Message.objects.create(match=safemeet.match, from_user=request.user, body=f"✅ Meeting confirmed for {safemeet.meeting_time.strftime('%B %d at %I:%M %p')} at {safemeet.location}!", is_system_message=True)
    messages.success(request, "✅ Meeting confirmed! Stay safe!")
    return redirect('safety_center')
...
def emergency_check_in_view(request, safemeet_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'check_in':
            ...
            safemeet.checked_in = True
            safemeet.save()
            messages.warning(request, "🚨 Emergency alert triggered.")
        return redirect('safety_center')
...
def api_matches_view(request):
    ...
    for m in matches:
        ...
        data.append({'id': m.id, 'display_name': other.profile.display_name})
...
def api_user_stats_view(request):
    u = request.user
    return JsonResponse({'tokens': 0, 'likes_received': Like.objects.filter(to_user=u).count(), 'total_matches': Match.objects.filter(Q(user_a=u) | Q(user_b=u)).count()})
